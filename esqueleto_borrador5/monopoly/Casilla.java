...
public class Casilla {
    ...
    public void setHipotecada(int hipotecada) {
 ...
 */
    ...
    public String casEnVenta(Jugador banca) {
        if (this.getDuenho() == null || this.getDuenho().getNombre().equalsIgnoreCase("Banca")) {
            return String.format("{tipo: %s, grupo: %s, valor: %.0f€}",
                    this.getTipo().toLowerCase(),
                    this.getGrupo() != null ? this.getGrupo().getColorGrupo() : "-", // usamos colorGrupo
                    (double) this.getValor());
        }
    ...
    public static void imprimir_en_venta(ArrayList<Casilla> casillas_lados, Jugador banca) {
        for (Casilla e : casillas_lados) {
            String enVenta = e.casEnVenta(banca);
            if (enVenta != null) {
    ...
    public static float getValor(String nombreConstante) {
        try {
            Field campo = Valor.class.getField(nombreConstante);
            return (float) campo.get(null);
        } catch (NoSuchFieldException | IllegalAccessException e) {
            System.out.println("No existe la constante: " + nombreConstante);
            return 0;
        }
    }

    // Devuelve el precio de un solar según el nombre de la casilla (ej: SOLAR1)
    public static float getPrecioSolar(String nombreCasilla) {
        return getValor(nombreCasilla.toUpperCase() + "_PRECIO");
    }

    // Devuelve la hipoteca de un solar según el nombre de la casilla
    public static float getHipotecaSolar(String nombreCasilla) {
        return getValor(nombreCasilla.toUpperCase() + "_HIPOTECA");
    }

    // Transporte
    public static float getPrecioTransporte() {
        return Valor.TRANSPORTE_PRECIO;
    }

    public static float getAlquilerTransporte() {
        return Valor.TRANSPORTE_ALQUILER;
    }

    // Servicio
    public static float getPrecioServicio() {
        return Valor.SERVICIO_PRECIO;
    }

    public static float getFactorServicio() {
        return Valor.FACTOR_SERVICIO;
    }

    // Impuestos
    public static float getValorImpuesto() {
        return Valor.IMPUESTO_VALOR;
    }

    // Salida de cárcel / parking
    public static float getCarcelSalida() {
        return Valor.CARCEL_SALIDA;
    }
}
