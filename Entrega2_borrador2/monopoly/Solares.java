...
public class Solares {
    ...
    public static class DatosEdificios {
        ...
        private int numHotel; //pode ser 0 ou 1
        private int numPiscina; //pode ser 0 ou 1
        private int numPista; //pode ser 0 ou 1
        private ArrayList<Edificacion> edificaciones; //ediciaciones construidas en esta casilla

        public DatosEdificios() {
            this.numCasas = 0;    // número de casas construídas (0..4)
            this.numHotel = 0;
            this.numPiscina = 0;
            this.numPista = 0;
        ...
        //Metodo para verificar se hai algunha edificacion
        public boolean tieneEdificios() {
            return numCasas > 0 || numHotel > 0 || numPiscina > 0 || numPista > 0 || !edificaciones.isEmpty();
        }
        ...
        public void setNumPiscina(int numPiscina) {
            this.numPiscina = numPiscina;
        }
    ...
    // Construir una casa
    public static boolean construirCasa(Casilla solar) {
        Solares.DatosEdificios estado = solar.getDatosEdificios();
        if (estado.getNumCasas() < 4 && estado.getNumHotel() == 0) {
            estado.setNumCasas(estado.getNumCasas() + 1);

            // Crear Edificación y añadirla a la lista
            Edificacion nuevaCasa = new Edificacion(Edificacion.Tipo.CASA, solar, obtenerPrecioCasa(solar.getNombre()));
            estado.getEdificaciones().add(nuevaCasa);
    ...
    // Construir un hotel (requiere 4 casas)
    public static boolean construirHotel(Casilla solar) {
        Solares.DatosEdificios estado = solar.getDatosEdificios();
        if (estado.getNumCasas() == 4 && estado.getNumHotel() == 0) {
            estado.setNumCasas(0);
            estado.setnumHotel(1);

            Edificacion nuevoHotel = new Edificacion(Edificacion.Tipo.HOTEL, solar, obtenerPrecioHotel(solar.getNombre()));
            estado.getEdificaciones().add(nuevoHotel);
    ...
    // Construir piscina
    public static boolean construirPiscina(Casilla solar) {
        Solares.DatosEdificios estado = solar.getDatosEdificios();
        if (estado.getNumPiscina() == 0) {
            estado.setNumPiscina(1);

            Edificacion nuevaPiscina = new Edificacion(Edificacion.Tipo.PISCINA, solar, obtenerPrecioPiscina(solar.getNombre()));
            estado.getEdificaciones().add(nuevaPiscina);
    ...
    // Construir pista de deporte
    public static boolean construirPista(Casilla solar) {
        Solares.DatosEdificios estado = solar.getDatosEdificios();
        if (estado.getNumPista() == 0) {
            estado.setNumPista(1);

            Edificacion nuevaPista = new Edificacion(Edificacion.Tipo.PISTA_DEPORTE, solar, obtenerPrecioPista(solar.getNombre()));
            estado.getEdificaciones().add(nuevaPista);
    ...
    // Demoler una casa
    public static boolean demolerCasa(Casilla solar) {
        Solares.DatosEdificios estado = solar.getDatosEdificios();
        if (estado.getNumCasas() > 0) {
            estado.setNumCasas(estado.getNumCasas() - 1);
            return true;
        }
        return false;
    ...
    // Demoler hotel
    public static boolean demolerHotel(Casilla solar) {
        Solares.DatosEdificios estado = solar.getDatosEdificios();
        if (estado.getNumHotel() > 0) {
            estado.setnumHotel(0);
            return true;
        }
        return false;
    }
    ...
    // Demoler piscina
    public static boolean demolerPiscina(Casilla solar) {
        Solares.DatosEdificios estado = solar.getDatosEdificios();
        if (estado.getNumPiscina() > 0) {
            estado.setNumPiscina(0);
            return true;
        }
        return false;
    }

    // Demoler pista de deporte
    public static boolean demolerPista(Casilla solar) {
        Solares.DatosEdificios estado = solar.getDatosEdificios();
        if (estado.getNumPista() > 0) {
            estado.setNumPista(0);
            return true;
        }
        return false;
    }
