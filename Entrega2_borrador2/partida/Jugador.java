...
public class Jugador {
    ...
    public void edificar(String tipoEdificio) {
        ...
        // Validar que la casilla sea Solar
        if (!"solar".equalsIgnoreCase(actual.getTipo())) {
            System.out.println("Non podes edificar en " + actual.getNombre() + ". Non é un solar.");
            return;
        }

        // Validar que eres el dueño
        if (actual.getDuenho() != this) {
            System.out.println("Non podes edificar en " + actual.getNombre() + ". Non es o dono.");
            return;
        }
        ...
        switch (tipoEdificio.toLowerCase()) {
            case "casa":
                coste = Solares.obtenerPrecioCasa(actual.getNombre());
                if (puedePagar(coste)) {
                    exito = Solares.construirCasa(actual);
                }
                break;
            case "hotel":
                coste = Solares.obtenerPrecioHotel(actual.getNombre());
                if (puedePagar(coste)) {
                    exito = Solares.construirHotel(actual);
                }
                break;
            case "piscina":
                coste = Solares.obtenerPrecioPiscina(actual.getNombre());
                if (puedePagar(coste)) {
                    exito = Solares.construirPiscina(actual);
                }
                break;
            case "pista":
                coste = Solares.obtenerPrecioPista(actual.getNombre());
                if (puedePagar(coste)) {
                    exito = Solares.construirPista(actual);
                }
                break;
            default:
                System.out.println("Tipo de edificacion inválido: " + tipoEdificio);
                return;
        }

        if (exito) {
            // Restamos dinero al jugador
            sumarFortuna(-coste);
            System.out.println("Edificación " + tipoEdificio + " construída en " + actual.getNombre() +
                    ". Fortuna actual: " + (int) getFortuna() + "€.");
        } else {
            System.out.println("Non se puido construir " + tipoEdificio + " en " + actual.getNombre() + ". Revisa requisitos ou edificacions existentes.");
        }
    }
