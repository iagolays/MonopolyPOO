...
public class Tablero {
    ...
    public Casilla encontrar_casilla(String nombre) {
        for (ArrayList<Casilla> lado : posiciones) { //recorremos todos os lados do tableiro
            for (Casilla c : lado) { //percorremos todas as casillas de cada lado
                if (c.getNombre().equalsIgnoreCase(nombre)) { //comprobamos que o nome da casilla coincide (en maiúsculas/minúsculas)
                    return c; //devolve a casilla encontrada
                }
            }
        }
        return null; //se non encontra devolve null
    }
